...
df = pd.read_csv(r"results\archive\profile_batch_matmul.20230522033549.csv")
df[['b', "i", "k", "j"]] = pd.DataFrame(
    columns=['b', "i", "k", "j"],
    data=df["shape"].map(eval).map(lambda x: (*x[0], x[1][2])).tolist(),
    index=df.index,
)

df['m'] = df.i*df.k*df.j * df.b
# df['m'] = (df.i*df.k  + df.j*df.k + df.i*df.j) * df.b
...
for op in grouped:
    ...
    print(df.sample(10)[['shape', 'b', 'i', 'j', 'k', 'log2_time', 'log2_pred_time', 'time', 'pred_time']])
