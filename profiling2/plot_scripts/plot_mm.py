...
LOWER_BOUND = int(sys.argv[1]) if len(sys.argv) == 3 else 4
UPPER_BOUND = int(sys.argv[2]) if len(sys.argv) == 3 else 10
...
df[['X']] = pd.DataFrame(columns=['X'], data=df['shape'].map(eval).map(lambda x: (*x[0], x[1][1])).to_numpy(), index=df.index)
...
exit()
def func(input, param):
    # (i, j, k, ij, jk, ik, ijk) = input
    # () = param
    res = 0
    # print(input, param)
    # print(len(input), len(param))
...
def residuals(p, z, inputs, func):
    """得到数据 z 和拟合函数之间的差"""
    return z - func(inputs, p)
...
inputs = [
    ...
    np.array(df['log2_jk']),
    np.array(df['log2_ik']),
    np.array(df['log2_ijk'])
]


init_param = np.array([1] * (len(inputs)+1))
z = df['log2_time']
plsq = optimize.leastsq(
    residuals, init_param, args=(z, inputs, func)
)  # 最小二乘法拟合
param = plsq[0] 
