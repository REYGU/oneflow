...
df = pd.read_csv(r"results\profile_reduce.20230526035418.csv")
df[['n', 'm']] = pd.DataFrame(columns=['m', 'n'], data=df['shape'].map(eval).map(lambda x: (*x,)).tolist(), index=df.index)
# df[['n', 'm']] = pd.DataFrame(columns=['m', 'n'], data=df['shape'].map(eval).map(lambda x: (*x[0],)).tolist(), index=df.index)
# df['m'] = df.i * df.k
# df['n'] = df.j
...
for _op in grouped:
    ...
    print("plane1: ", [param1[0], param1[1], -1, param1[2]])
    print("plane2: ", a)
    print("plane3: ", b)
    print("line: ", param4)
