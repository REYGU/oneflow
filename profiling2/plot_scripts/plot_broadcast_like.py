...
df = pd.read_csv(r"results/profile_broadcast_like.20230524034101.csv")

df[['m', 'n']] = pd.DataFrame(columns=['m', 'n'], data=df['shape'].map(eval).map(lambda x: (*x[1],)).tolist(), index=df.index)
# df['m'] = df.i * df.k
# df['n'] = df.j
