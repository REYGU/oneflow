...
df = df[df.op.apply(lambda x: x.startswith("broadcast"))]


def parse_shape(shape):
    x = eval(shape)
    m1, n1 = (1, x[0][0]) if len(x[0]) == 1 else (*x[0],)
    m2, n2 = (1, x[1][0]) if len(x[1]) == 1 else (*x[1],)
    assert n1 == n2
    m = max(m1, m2)
    return m1, m2, m, n1


df[["m1", "m2", "m", "n"]] = pd.DataFrame(
    columns=["m1", "m2", "m", "n"],
    data=df["shape"].map(parse_shape).tolist(),
    index=df.index,
)
# df[['m1', 'n1', 'm2', 'n2']] = pd.DataFrame(columns=['m1', 'n1', 'm2', 'n2'], data=df['shape'].map(eval).map(lambda x: (*x[0], *x[1])).tolist(), index=df.index)
# df['m'] = df.i * df.k
# df['n'] = df.j
# df['m'] = max(df['m1'], df['m2'])
# df['n'] = df.n1

# df = pd.read_csv(r"results/profile_layernorm.20230524071853.csv")
# df[['m', 'n']] = pd.DataFrame(columns=['m', 'n'], data=df['shape'].map(eval).map(lambda x: (*x,)).tolist(), index=df.index)
...
def piecewise_plane_func2(x, y, param1, param3):
    vinp1 = plane_func_3var(x, y, param1)
    vinp3 = plane_func_3var(x, y, param3)

    z = param1[2]
    value = vinp1 * ((vinp3 <= z)) + vinp3 * ((vinp3 > z))

    return value


def piecewise_plane_classify(x, y, param1, param3):
    z = param1[2]
    mask = [
        ( (plane_func_3var(x, y, param3) <= z)),
        ((plane_func_3var(x, y, param3) > z)),
    ]
    return mask
...
def eval_picecwise_plane(df, params):
    ...
    pred_z = piecewise_plane_func2(x, y, *params)
    # print("-" * 150)
...
(param1, param3) = fit_piecewise_plane(sf)
# a, b = (1, -1, 1, 2), (-1, 1, 1, 3)
...
y = np.arange(0, 30, 1)  # y定义域，离散
X, Y = np.meshgrid(x, y)
Z = piecewise_plane_func2(X, Y, *params)  # 带入拟合得到的 a, b
# ax.plot_surface(X, Y, Z, rstride=1, cstride=1, cmap="rainbow")
